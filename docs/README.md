...
- **Anonymous accounts + cross-device sync** — done: the per-device id is the anonymous account key (no login). A `/sync` page mints a short-lived, single-use **sync code** (`sync.ts`; pure code/expiry logic in `syncLogic.ts`) that another device redeems to adopt the same account — saves, streak, and personalization follow. Codes expire in 15 min, are retired when a new one is minted, and fail loudly on unknown/expired/used. Better Auth + Google/Apple (codeless multi-device) remains the future upgrade this slots into.
