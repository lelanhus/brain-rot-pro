...
- `profileLogic.scoreCard` only subtracts a `SEEN_PENALTY` — seen cards are *down-ranked, not excluded*, so repeats happen.
...
- **Light personalization now, AI-ranking later:** start with concept-affinity + variety; structure the feed as *select unseen → rank* so an embedding/taste-vector ranker can replace the ranker without a rewrite (cards already carry embeddings).
