...
The top-1000/day API is recency/popularity-skewed *regardless* of how many days you harvest, and `fetch failed` from Convex makes it unreliable. The raw hourly pageview dumps list every viewed article with counts — running locally (reliable networking, no Convex action timeouts), sampled across months, yields a large, diverse, evergreen-rich topic set.
