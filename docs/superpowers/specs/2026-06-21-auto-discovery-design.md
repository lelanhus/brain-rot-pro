...
## Goal
Broaden a user's interests automatically: when they follow a topic (explicit), find a few *related* catalog topics and add them as `discovered` interests — so the blended feed (SP3) and generation (SP2) widen without manual effort.
